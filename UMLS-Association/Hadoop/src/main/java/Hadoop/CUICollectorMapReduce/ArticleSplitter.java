...
/**
 * ArticleSplitter processes MetaMap output files in order to condense all utterances from the same 
...
public class ArticleSplitter {
	...
	public static class ArticleReducer extends Reducer<Text,Text,Text,Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			multipleOutputs.write(key, result, key.toString());
		}
	...
	/**
	 * The main method for running the ArticleSplitter.  Sets up job configuration, defines classes to use,
	...
	public static void main(String[] args) throws Exception {
		...
		try{
			...
			formatter.printHelp("hadoop jar <path/to/jar/file.jar> Hadoop.CUICollectorMapReduce.ArticleSplitter -i <input> -o <outdir>", options);
			System.exit(1);
			return;
		}
		...
		job.setJarByClass(ArticleSplitter.class);
