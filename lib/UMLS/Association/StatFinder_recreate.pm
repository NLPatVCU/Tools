...
sub _getCUICooccurrences_matrix {
	...
	foreach my $key(keys %{$matrixRef}) {
		...
		$cooccurrences2{$cui2} = ${$matrixRef}{$key};  #TODO, this is the "bug" from before which gives us good results. We add $cui2 to the list of co-occurrences. The result of this is that when matching cui co-occurrences, we see OK, cui2 with cui2 and cui1 with cui2, so we take the min of those two values. This essentially makes the measure LIKE association scores (no implicit stuff involved). It is a bug though cause cui2 
	    } 
	}
    }
