...
sub _initialize {
    ...
    #precompute values if using an input matrix
    #  but do not need to precompute when using LTA
    #  rather than normal assocaition
    if ($matrix_G && !$lta_G) {
	$self->_precomputeValues();
    }

    #precompute LTA values (right now just npp)
    if ($lta_G) {
...
# Precalculation step that makes using a matrix faster for multiple CUIs over
# a database, by pre calculating N11 for all cui pairs, and NP1 and N1P for 
# all cuis, and NPP in a single pass of the matrix file
# input: none
# output: none, but global all matrix stats are set (n11, n1p, np1, npp)
sub _precomputeValues {
    my $self = shift;
...
# Precalculation step for LTA, creates n1p, np1, and npp
# n1p and np1 are comma seperated lists of co-occurring cuis
# input: none
# output: none, but global npp is set
sub _precomputeValues_LTA() {
    my $self = shift;

    #calculate npp with DB or matrix
    if ($matrix_G) {
	#npp is the number of unique cuis
	my %uniqueCuis = ();
	%n1pAll_G = ();
	%np1All_G = ();
	...
	$npp_G = scalar keys %uniqueCuis;
    }
    else {
	#TODO, is this correct, it seems like cui_1 and cui_2 have the same 
	# number of keys in the DB,  which is weird maybe?
	#get npp, the count of unique cuis
	my $db = $cuifinder_G->_getDB(); 
	my $npp = shift $db->selectcol_arrayref("select count(cui_1) from N_11"); 
    }
}
...
# calculate a contingency table values and a statistic (associationScore)
# for the cui pair
...
sub calculateStatistic { 
	...
	if ($matrix_G) {
	    $valid = $self->getStats_matrix(\@cuis1, \@cuis2);
	}
	else {
    ...
    my $stat = $self->calculateStatistic(
	$n11, $n1p, $np1, $npp, $statistic);
    return $stat;
}

# calculates a statistic (association score) from the provided values
...
sub _getStats_matrix {
    ...
    #grab data
    my $n11 = $self->_getN11_matrix($cuis1Ref, $cuis2Ref); 
    my $n1p = $self->_getN1p_matrix($cuis1Ref); 
    my $np1 = $self->_getNp1_matrix($cuis2Ref); 
...
sub _getStats_LTA {
    ...
    if ($matrix_G) {
	($cooccurrences1Ref, $cooccurrences2Ref) = &_getCUICooccurrences_matrix($cuis1Ref, $cuis2Ref);
    }
    else {
	($cooccurrences1Ref, $cooccurrences2Ref) = &_getCUICooccurrences_DB($cuis1Ref, $cuis2Ref);
    }
...
#TODO, problem here cause we assume we have a comma seperated list of cui co-occurrences, which we dont...but we could build in the beginning
    #get lists of explicitly co-occurring CUIs for each concept
    #add trailing cui co-occurrences to cui1Data
    my %cooccurrences1;
    foreach my $cui1 (@{$cuis1Ref}){
	if (defined $n1pAll_G{$cui1}) {
	    foreach my $cui (split /,/,$n1pAll_G{$cui1}) {
    ...
    my %cooccurrences2;
    foreach my $cui2 (@{$cuis2Ref}) {
	if (defined $np1All_G{$cui2}) {
	    foreach my $cui (split /,/,$np1All_G{$cui2}) {
	...
	foreach my $cui1 (@{$cuis1Ref}) {
	    if (defined $np1All_G{$cui1}) {
		foreach my $cui (split /,/,$np1All_G{$cui1}) {
		    $cooccurrences1{$cui} = 1;
		}
	    }
	}
	...
	foreach my $cui2 (@{$cuis2Ref}) {
	    if(defined $n1pAll_G{$cui2}) {
		foreach my $cui (split /,/,$n1pAll_G{$cui2}) {
		    $cooccurrences2{$cui} = 1;
		}
	    }
	}
