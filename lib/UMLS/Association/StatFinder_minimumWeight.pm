...
########################################################################
# functions to get statistical information about the cuis LTA and MWA
########################################################################
...
sub _getStats_LTA {
    ...
    (my $cooccurrences1ListRef, my $cooccurrences2ListRef,  my $numCooccurrences, my $numUniqueCuis) = $self->_readMatrixValues_LTA_MWA($pairHashListRef); 
...
#  Gets contingency table values for MWA
#  input : $pairHashListRef <- ref to a pairHashList
#  output: $\@data  <- array ref containing array refs of four values
#                      for each pairHash in the pairHash list. The 
#                      values are $n11, $n1p, $np1, and $npp
sub _getStats_MWA {
	...
	my $n1p = 0;
	foreach my $cui (keys %{${$cooccurrences1ListRef}[$i]}) {
	    $n1p += ${${$cooccurrences1ListRef}[$i]}{$cui};
	}
	my $np1 = 0;
	foreach my $cui (keys %{${$cooccurrences2ListRef}[$i]}) {
	    $np1 += ${${$cooccurrences2ListRef}[$i]}{$cui};
	}
	...
	foreach my $cui (keys %{${$cooccurrences1ListRef}[$i]}) {
	    if (exists ${${$cooccurrences2ListRef}[$i]}{$cui}) {
		my $min = ${${$cooccurrences1ListRef}[$i]}{$cui};
		if (${${$cooccurrences2ListRef}[$i]}{$cui} < $min) {
		    $min = ${${$cooccurrences2ListRef}[$i]}{$cui};
		}
		$n11+=$min;
	    }
	}
...
#  input : $pairHashListRef <- ref to a pairHashList
#  output: $\@data  <- array ref containing array refs of four values
sub _readMatrixValues_LTA_MWA {
    #grab parameters
    my $self = shift;
    my $pairHashListRef = shift;
    
    #error checking
    my $function = "_readMatrixValues_LTA"; 
    if(!defined $self || !ref $self) {
        $errorhandler->_error($pkg, $function, "", 2);
    }
	...
	(my $cuis1Ref, my $cuis2Ref) = $self->_getAllLeadingAndTrailingCuis($pairHashListRef);
	(my $n1pAllRef, $np1AllRef, $numCooccur, $numUniquCuis) = $self->_getObserved_matrix_LTA_MWA($cuis1Ref, $cuis2Ref);
...
sub _getObserved_matrix_LTA_MWA {
    #grab parameters
    my $self = shift;
    my $cuis1Ref = shift;
    my $cuis2Ref = shift;
	...
	#update n1pAll and np1All order matters
	if (exists $cuis1{$cui1} || exists $cuis2{$cui2}) {
	    ...
	    #add the cui2 hash with value
	    my %cui2Hash = ();
	    $cui2Hash{$cui2} = $num;
	    $n1pAll{$cui1} = \%$cui2Hash;

	    #update np1All
	    #create np1{$cui2} hash if needed
	    if (!defined $np1All{$cui1}) {
		my %newHash = ();
		$np1All{$cui2} = \%newHash;
	    }

	    #add the cui1 hash with value
	    my %cui1Hash = ();
	    $cui1Hash{$cui1} = $num;
	    $np1All{$cui2} = \%$cui1Hash;
	}

	#update n1pAll and n1pAll order doesnt matter
	# Note: this code is the same as above, but 
	# flipped so that order doesn't matter
	if ($noOrder_G) {
	    if (exists $cuis2{$cui1} || exists $cuis1{$cui2}) {
		#update n1pAll
		#create n1p{$cui2} hash if needed
		if (!defined $n1pAll{$cui2}) {
		    my %newHash = ();
		    $n1pAll{$cui2} = \%newHash;
		}
		...
		my %cui1Hash = ();
		$cui1Hash{$cui1} = $num;
		$n1pAll{$cui2} = \%$cui12Hash;

		#update np1All
		#create np1{$cui1} hash if needed
		if (!defined $np1All{$cui1}) {
		    my %newHash = ();
		    $np1All{$cui1} = \%newHash;
		}
...
sub _getCUICooccurrences_matrix {
	...
	if (defined ${$n1pAllRef}{$cui1}) {
	    foreach my $cui2 (${$n1pAllRef}{$cui1}) {
	...
	if (defined ${$np1AllRef}{$cui2}) {
	    foreach my $cui2 (${$np1AllRef}{$cui2}) {
	...
	foreach my $cui1 (@{$cuis1Ref}){
		...
		foreach my $cui2 (${$np1AllRef}{$cui1}) {
	...
	foreach my $cui2 (@{$cuis2Ref}){
		...
		foreach my $cui2 (${$n1pAllRef}{$cui2}) {
